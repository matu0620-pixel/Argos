...
// All client.messages.create(...) calls go through lib/gemini-client.js which
// emulates the Anthropic SDK surface so the analyze logic is unchanged.
...
export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: "config_error", message: "GEMINI_API_KEY 未設定" });
  ...
  try {
    ...
    // Phase 4 (Investment Thesis) — Coverage Tier は EDINET 充実度で判定
    const hasFullEdinet = (edinetFinancials?.length || 0) >= 3;
    const hasPartialEdinet = (edinetFinancials?.length || 0) >= 1 && (edinetFinancials?.length || 0) < 3;

    let coverageTier;
    if (hasFullEdinet) coverageTier = "full";       // EDINET ≥3 期 → 最高品質
    else if (hasPartialEdinet) coverageTier = "limited"; // EDINET 1-2 期 → 限定的
    else coverageTier = "draft";                    // EDINET 取得失敗 → 初期仮説

    const defaultConviction = {
      full: "high", limited: "medium", draft: "low",
    }[coverageTier];
    ...
    try {
      ...
      const p4 = await fetchPhase(client, buildPromptPhase4(code, authoritativeName, ctxSummary, indCtxStr, null, {
        coverageTier,
    ...
    try {
      ...
      const p5Prompt = buildPromptPhase5(code, authoritativeName, ctxForP5);
