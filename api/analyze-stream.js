// api/analyze-stream.js — Streaming with EDINET financials + Gemini for non-financial data
// Migrated 2026-05 from Anthropic Claude → Google Gemini 2.5 Flash (free tier).
// All client.messages.* calls go through lib/gemini-client.js compatibility shim.
...
async function streamPhase(client, send, label, prompt, maxUses) {
  const stream = await client.messages.stream({
    ...
    tools: [{ type: "google_search", name: "web_search", max_uses: maxUses }]
  });

  let searchCount = 0, textLength = 0, lastReported = 0;

  for await (const event of stream) {
    // Note: Gemini stream doesn't emit per-search events the way Claude did.
    // We synthesize a single search progress event when we detect grounding
    // metadata in the stream (handled inside gemini-client.js).
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    res.status(500).json({ error: "config_error", message: "GEMINI_API_KEY 未設定" });
    return;
  }
  ...
  try {
    ...
    /* ─── PHASE 4: Institutional Investment Thesis (synthesis) ───
       Coverage tier は EDINET 充実度で判定。
       ──────────────────────────────────────────────────────────── */
    let p4SearchCount = 0;

    // ─── Coverage tier classification (EDINET 充実度ベース) ───
    //   "full"    — EDINET ≥ 3 期 → 🏆 FULL COVERAGE
    //   "limited" — EDINET 1-2 期 → 🟡 LIMITED DATA
    //   "draft"   — EDINET 取得失敗 → 🟡 INITIAL DRAFT
    ...
    let coverageTier;
    if (hasFullEdinet) coverageTier = "full";          // EDINET ≥ 3 期
    else if (hasPartialEdinet) coverageTier = "limited"; // EDINET 1-2 期
    else coverageTier = "draft";                       // EDINET なし
    ...
    // For backward compat
    const thesisSource = ({
      full: "edinet", limited: "limited", draft: "draft",
    })[coverageTier];
    ...
    // Always run Phase 4 — even with no data we generate an "initial draft" thesis
    {
      ...
      const tierSources = {
        full:    `EDINET ${edinetFinancials.length} 期 + 業種中央値`,
        edinet:  `EDINET ${edinetFinancials.length} 期 + 業種中央値 + 過去バリュエーション`,
        limited: `EDINET ${edinetFinancials?.length || 0} 期 + 業界知識`,
      ...
      try {
        ...
        const p4 = await streamPhase(
          client, send, "phase4",
          buildPromptPhase4(code, authoritativeName, ctxSummary, indCtxStr, null, {
            coverageTier,
