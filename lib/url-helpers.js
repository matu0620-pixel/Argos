...
export function sanitizeSourceChips(chips, code) {
  ...
  return chips
    ...
    .map(c => {
      ...
      // 0) "会社 IR" / "会社開示一覧" chips are handled by the EDINET-anchored
      //    verification step in the API merge — leave them alone here so we
      //    don't lock in an unverified URL via the heuristic fallback.
    ...
    .filter(c => {
      // Drop chips with empty URLs unless they're "会社 IR" pending verification
      if (!c.url && /会社 ?IR|会社開示一覧/.test(c.label || "")) return true;
      return isValidHttpUrl(c.url);
    });
}
