   ...
   ─────────────────────────────────────────────────────────
{
  ...
  price: 749,
  change_pct: 0.94,
  conviction: "high",         // best-effort（解析時点のテーゼ確信度）
  verdict_at_time: "buy",     // best-effort
...
export async function saveEntry(deviceId, code, merged, extra = {}) {
  if (!isValidDeviceId(deviceId)) throw new Error("Invalid deviceId");
  if (!/^(\d{4,5}|\d{3}[A-Z])$/i.test(String(code))) throw new Error("Invalid code");
