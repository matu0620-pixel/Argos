...
/* ─────────────────────────────────────────────────────────
   Bench 追加指標: CF（営業/投資）・BS（現預金/有利子負債・Instant）・平均年収
   ※ 当期/前期の2期分のみ（詳細財務諸表）。
   ───────────────────────────────────────────────────────── */
export function extractBenchExtras(rows) {
  ...
  const DURATION = {
    operating_cf: ["jppfs_cor:NetCashProvidedByUsedInOperatingActivities", "jpigp_cor:NetCashProvidedByUsedInOperatingActivitiesIFRS"],
    investing_cf: ["jppfs_cor:NetCashProvidedByUsedInInvestmentActivities", "jpigp_cor:NetCashProvidedByUsedInInvestingActivitiesIFRS"],
  };
  const INSTANT_CASH = ["jppfs_cor:CashAndDeposits", "jppfs_cor:CashAndCashEquivalents", "jpigp_cor:CashAndCashEquivalentsIFRS"];
  const INSTANT_DEBT = [
    "jppfs_cor:ShortTermLoansPayable", "jppfs_cor:CurrentPortionOfLongTermLoansPayable",
    "jppfs_cor:LongTermLoansPayable", "jppfs_cor:BondsPayable", "jppfs_cor:CurrentPortionOfBonds",
    "jppfs_cor:ShortTermBondsPayable", "jppfs_cor:CommercialPapersLiabilities",
    "jppfs_cor:CurrentPortionOfBondsPayable", "jppfs_cor:LeaseObligationsCL", "jppfs_cor:LeaseObligationsNCL",
  ];
  const SALARY = "jpcrp_cor:AverageAnnualSalaryInformationAboutReportingCompanyInformationAboutEmployees";

  const offDur = (ctx) => ctx && ctx.includes("CurrentYearDuration") ? 0 : (ctx && /Prior(\d)YearDuration/.test(ctx) ? Number(ctx.match(/Prior(\d)YearDuration/)[1]) : null);
  const offInst = (ctx) => ctx && ctx.includes("CurrentYearInstant") ? 0 : (ctx && /Prior(\d)YearInstant/.test(ctx) ? Number(ctx.match(/Prior(\d)YearInstant/)[1]) : null);

  const byYear = {};
  const ensure = (o) => (byYear[o] = byYear[o] || {});
  let average_salary = null;

  for (const row of rows) {
    const el = getCol(row, "要素ID", "要素 ID", "element ID", "Element ID");
    if (!el) continue;
    const ctx = getCol(row, "コンテキストID", "コンテキスト ID", "context ID", "Context ID");
    const cflag = getCol(row, "連結・個別", "連結個別", "consolidated, individual");
    const unit = getCol(row, "ユニットID", "ユニット ID", "unit ID", "Unit ID");
    const valRaw = getCol(row, "値", "value", "Value");
    if (!valRaw) continue;
    const num = Number(valRaw);
    if (!Number.isFinite(num)) continue;
    if (cflag === "個別" || cflag === "単体") continue; // 連結優先

    if (el === SALARY) { if (average_salary == null) average_salary = num; continue; }
    if (unit && !unit.includes("JPY") && !unit.includes("円")) continue;
    const mil = Math.round(num / 1e6);

    const dmetric = Object.keys(DURATION).find((m) => DURATION[m].includes(el));
    if (dmetric) {
      const o = offDur(ctx); if (o == null) continue;
      const y = ensure(o); if (y[dmetric] == null) y[dmetric] = mil;
      continue;
    }
    if (INSTANT_CASH.includes(el)) {
      const o = offInst(ctx); if (o == null) continue;
      const y = ensure(o); if (y.cash == null) y.cash = mil;
      continue;
    }
    if (INSTANT_DEBT.includes(el)) {
      const o = offInst(ctx); if (o == null) continue;
      const y = ensure(o); y.interest_bearing_debt = (y.interest_bearing_debt || 0) + mil;
      continue;
    }
  }
  return { byYear, average_salary };
}
...
export async function getFinancialsByCode(apiKey, code) {
  ...
  const annual = yearOffsets.map(offset => {
    ...
    const cogs = y.cost_of_sales;
    let gp = y.gross_profit;
    const sga = y.sga;
    // 売上総利益が無くても 売上高−売上原価 で補完
    if (gp == null && rev != null && cogs != null) gp = rev - cogs;
    const pctOf = (a) => (rev && a != null) ? Math.round((a / rev) * 1000) / 10 : null;

    return {
      fy: fyLabelFor(offset),
      year_offset: offset,
      revenue: rev ?? null,
      cost_of_sales: cogs ?? null,
      gross_profit: gp ?? null,
      sga: sga ?? null,
      operating_profit: op ?? null,
      ordinary_profit: ord ?? null,
      net_profit: np ?? null,
      cogs_ratio: pctOf(cogs),
      gp_margin: pctOf(gp),
      sga_ratio: pctOf(sga),
  ...
  // ── Bench 追加指標: CF（営業/投資→FCF）・BS（現預金/有利子負債→ネットキャッシュ）・平均年収 ──
  // （ARGOS Bench の bench-financials が financials_annual から参照）
  try {
    const extras = extractBenchExtras(rows);
    companyInfo.average_salary = extras.average_salary ?? null; // 円
    for (const a of annual) {
      const ex = extras.byYear[a.year_offset] || {};
      a.operating_cf = ex.operating_cf ?? null;
      a.investing_cf = ex.investing_cf ?? null;
      a.free_cash_flow = (ex.operating_cf != null && ex.investing_cf != null) ? ex.operating_cf + ex.investing_cf : null;
      a.cash = ex.cash ?? null;
      a.interest_bearing_debt = ex.interest_bearing_debt ?? null;
      a.net_cash = (ex.cash != null && ex.interest_bearing_debt != null) ? ex.cash - ex.interest_bearing_debt : null;
    }
  } catch (e) {
    console.warn("[EDINET] extractBenchExtras failed:", e.message);
  }
