 ...
 * Returns rows in the {key, val, small, extra} schema used by frontend.
 * "extra" includes "EDINET" tag to indicate authoritative source.
 */
export function buildEdinetListingRows(info) {
  ...
  if (info.founded_date) {
    rows.push({
      key: "設立",
      val: formatJapaneseDate(info.founded_date),
      extra: "EDINET",
      _src: "edinet",
    });
  }
  ...
  if (info.audit_firm) {
    let firm = String(info.audit_firm).trim();
    // Trim if too long
    if (firm.length > 30) firm = firm.slice(0, 28) + "…";
    rows.push({
      key: "監査法人",
      val: firm,
      extra: "EDINET",
