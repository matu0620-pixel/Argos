...
【証券コードの形式】
東証の証券コードは 2 種類あります。両方を正しく扱ってください:
- **従来形式**: 4-5 桁の数字 (例: 7203 トヨタ、9984 ソフトバンクG、13010 マルハニチロ)
- **新形式 (2024〜)**: 3 桁数字 + 1 文字英字 (例: 173A ハンモック、184A エコム)
  → 新規上場が増えて 4 桁数字が枯渇したため、TSE は 2024 年から新形式コードを採用
  → Yahoo!ファイナンス URL は https://finance.yahoo.co.jp/quote/173A.T のように使われる
...
【特別ルール】
- 入力が既に 4-5 桁の数字、または 3 桁数字 + 1 文字英字 (例: 173A) なら、それを code として返す
 ...
 *
 * @param {object} client - Gemini client instance (lib/gemini-client.js)
...
export async function resolveCompanyToCode(client, input) {
  ...
  // Fast path: already a code (legacy 4-5 digit OR new 3-digit + letter format like 173A)
  const upperTrimmed = trimmed.toUpperCase();
  if (/^(\d{4,5}|\d{3}[A-Z])$/.test(upperTrimmed)) {
    return { code: upperTrimmed, name_jp: "", name_en: "", market: "", confidence: "high", candidates: [] };
  }
  ...
  try {
    response = await client.messages.create({
      ...
      max_tokens: 800,
      tools: [{ type: "google_search", name: "web_search", max_uses: 3 }],
  ...
  // Validate code format (legacy 4-5 digit OR new 3-digit + letter format like 173A)
  let code = parsed.code ? String(parsed.code).trim().toUpperCase() : null;
  if (code && !/^(\d{4,5}|\d{3}[A-Z])$/.test(code)) {
  ...
  // Sanitize candidates list — accept both legacy and new format codes
  const candidates = Array.isArray(parsed.candidates)
    ? parsed.candidates
        .filter(c => c && c.code && /^(\d{4,5}|\d{3}[A-Z])$/i.test(String(c.code).trim()))
        .map(c => ({
