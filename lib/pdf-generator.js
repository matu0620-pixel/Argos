...
const COVERAGE_TIERS = {
  full:    { color: C.gold,  label: "FULL COVERAGE",   icon: "♛", desc: "EDINET 3 期分の財務" },
  edinet:  { color: C.gain,  label: "EDINET-VERIFIED", icon: "✓", desc: "EDINET 3 期分の財務" },
  limited: { color: C.warn,  label: "LIMITED DATA",    icon: "○", desc: "EDINET 1-2 期分" },
...
function pageBusinessProfile(doc, d) {
  ...
  if (d.company?.blurb) {
    ...
    // Blurb source indicator
    const src = d._blurb_source;
    if (src) {
      let label = "AI 生成";
      let color = C.text4;
      if (src.startsWith("Shikiho")) {
        label = "出典: 会社四季報「特色」";
        color = C.gold;
      } else if (src === "EDINET") {
        label = "出典: EDINET 有報「事業の内容」";
        color = C.cyan;
      }
      doc.font("JP").fontSize(7).fillColor(color).text(`◆ ${label}`, lx + 16, y + 2, { width: lw - 32, characterSpacing: 0.5 });
      y = doc.y + 8;
    } else {
      y += 12;
    }
...
// Format a row of financial values consistently, choosing the right unit (億円 or 百万円)
// based on the magnitude of the largest value in the row.
//
// IMPORTANT: EDINET values are stored in 百万円 (millions of yen) by lib/edinet.js
// (see line 792: valueInMillions = numValue / 1_000_000). So we treat each `v` as
// already-in-百万円 and only convert UP to 億円 when values are large.
function formatFinancialRow(values) {
  // Filter to numeric values for max calculation
  const numerics = values.filter(v => v != null && Number.isFinite(v)).map(v => Math.abs(v));
  if (numerics.length === 0) {
    return { unit: "百万円", formatted: values.map(() => "—") };
  }
  const maxAbs = Math.max(...numerics);

  // Decision tree (values are in 百万円):
  //   maxAbs >= 100,000 (= 1,000 億)            → 億, no decimals
  //   maxAbs >= 10,000  (= 100 億)             → 億, 1 decimal
  //   maxAbs >= 1,000   (= 10 億)              → 億, 1 decimal
  //   maxAbs >= 100                             → 億, 2 decimals (e.g. 1.23 億)
  //   else                                      → 百万円
  let unit, divisor, decimals;
  if (maxAbs >= 10000) {
    // ≥ 100 億 → display in 億, no decimals
    unit = "億円"; divisor = 100; decimals = 0;
  } else if (maxAbs >= 1000) {
    // 10 億 〜 100 億 → display in 億, 1 decimal
    unit = "億円"; divisor = 100; decimals = 1;
  } else if (maxAbs >= 100) {
    // 1 億 〜 10 億 → display in 億, 2 decimals
    unit = "億円"; divisor = 100; decimals = 2;
  } else {
    // < 1 億 → display in 百万円
    unit = "百万円"; divisor = 1; decimals = 0;
  }
...
function pageFinancials(doc, d) {
  ...
  // EDINET annual data: array of { fy, revenue, operating_profit, ordinary_profit, net_profit }
  // All values are stored in 百万円 by lib/edinet.js (line 792).
  ...
  const metrics = [
    ...
    { label: "営業利益",   key: "operating_profit" },
    { label: "経常利益",   key: "ordinary_profit" },
    { label: "当期純利益", key: "net_profit" },
  ];

  // Pre-compute formatted values + per-row unit for each metric
  const rowsFormatted = metrics.map(m => {
    const values = annualData.map(y => y[m.key]);
    const { unit, formatted } = formatFinancialRow(values);
    return { ...m, unit, values: formatted };
  });
  ...
  rowsFormatted.forEach((m, idx) => {
    ...
    // Show the unit determined for THIS row (consistent across all years)
    doc.fontSize(7).fillColor(C.text4).text(`(${m.unit})`, 50, dy + 14, { width: 180 });
    m.values.forEach((txt, i) => {
