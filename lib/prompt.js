...
【出力 JSON】 JSON のみ。前置き・コードフェンス禁止:
{
 "investment_thesis":{
  ...
  "catalysts":[
   今後 12 ヶ月の重要イベント 4-6 件
   {
    "date_label":"YYYY/MM 上旬|中旬|下旬 or 'YYYY/MM/DD'",
    "type":"earnings|regulatory|strategic|macro",
    "type_label":"決算|規制|戦略|マクロ",
    "event":"イベント名 (10-30 文字)",
    "expected_impact":"high|mid|low",
    "direction":"up|down|neutral",
    "thesis_relevance":"テーゼに対する重要度の説明 1 文"
   }
  ],
...
export function postProcessPhase1(data) {
  ...
  if (Array.isArray(data.ir_news)) {
    ...
    data.ir_news.forEach(n => stripNewlines(n, ["title", "meta", "label"]));
  }
