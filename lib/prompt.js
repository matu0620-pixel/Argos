...
【URL 出力の絶対ルール (極めて重要)】
- IR ニュース個別の URL フィールドは廃止 (ir_news の各要素には url を含めない)
- 代わりに、ページ上部の "company_ir_url" に **会社公式 IR ページのトップ** (例: https://www.infomart.co.jp/ir/) を 1 つだけ記載する
...
- TDnet (release.tdnet.info) の特定開示への deep link 出力禁止 — 31 日経過で 404 になる
...
【出力 JSON】 JSON のみ:
{
 ...
 "ir_news":[新→古順, 6-9 件 — Phase 1 では仮取得。後段で会社 IR サイトを直接検索して上書きされる場合あり {
  "date":"YYYY.MM.DD",
  "fy":"FYxx Qn",
...
export function postProcessPhase1(data) {
  ...
  if (Array.isArray(data.ir_news)) {
    ...
    data.ir_news.forEach(n => {
      ...
      // Per-news URL field is deprecated — strip it
      delete n.url;
      delete n._url_source;
    });
  }
