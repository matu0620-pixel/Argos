...
/* ─────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────── */
export function buildPromptPhase4(code, companyName, contextSummary, industryContext, memoContext = null, coverage = null) {
  const company = companyName ? `「${companyName}」 (証券コード ${code})` : `証券コード ${code}`;
  const indCtx = industryContext ? `\n${industryContext}\n` : "";
...
` : "";
  ...
  // Coverage tier specific instructions（EDINET 充実度ベース）
...
★★★ INITIAL DRAFT モード ★★★
ユーザーがこの銘柄を初めて分析しています。EDINET 財務が取得できていません。
...
★ LIMITED DATA モード ★
EDINET ${coverage.edinetYears} 期 のみ。
...
★★ FULL COVERAGE モード ★★
EDINET 財務 ${coverage.edinetYears} 期、最高品質の Thesis を作成:
...
/**
 * Repairs common *mid-stream* JSON syntax errors that LLMs (Gemini) frequently emit.
 * Unlike repairTruncatedJson (which only closes unterminated braces/brackets at the END),
 * this scans the whole document and fixes:
 *   1. Missing commas between adjacent values  ["a" "b"]  →  ["a","b"]   and  {"a":1 "b":2} → {"a":1,"b":2}
 *   2. Trailing commas before } or ]            [1,2,]    →  [1,2]
 *   3. Raw control chars inside strings (newlines/tabs) that break JSON.parse
 * It is string-aware so it never alters content inside quoted strings.
 */
export function repairJsonSyntax(s) {
  let out = "";
  let inStr = false, esc = false;
  let afterValue = false; // last completed token was a value (string/}/]/number/true/false/null)
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (inStr) {
      ...
      if (c === '"') { inStr = false; out += c; afterValue = true; continue; }
      // Escape raw control characters that would otherwise be invalid inside a JSON string
      if (c === "\n") { out += "\\n"; continue; }
    ...
    // Whitespace outside strings — pass through untouched
    ...
    // A value just ended and a new value begins with no separator → insert the missing comma
    if (afterValue && startsValue) {
      out += ",";
      afterValue = false;
    }

    if (c === '"') { inStr = true; afterValue = false; out += c; continue; }
    if (c === "{" || c === "[") { afterValue = false; out += c; continue; }
    if (c === "}" || c === "]") {
      out = out.replace(/,(\s*)$/, "$1"); // strip trailing comma before close
      afterValue = true; out += c; continue;
    }
    if (c === "," || c === ":") { afterValue = false; out += c; continue; }

    // Bare value characters (digits, -, +, ., e/E, and true/false/null letters)
    out += c;
    afterValue = /[0-9.eEltrufas+]/.test(c);
  }
...
export function parseResponseJson(text) {
  ...
  const strategies = [
    s => s,
    s => repairJsonSyntax(s),
    s => repairTruncatedJson(s),
    s => repairTruncatedJson(repairJsonSyntax(s)),
    s => repairJsonSyntax(repairTruncatedJson(s)),
  ];

  let firstError = null;
  for (let i = 0; i < strategies.length; i++) {
    try {
      const data = JSON.parse(strategies[i](jsonStr));
      return { data, repaired: i > 0 };
    } catch (e) {
      if (i === 0) firstError = e;
    }
  }
...
/* ─── Phase 5: Report Authoring ───────────────────────────
   ...
   ───────────────────────────────────────────────────────── */
export function buildPromptPhase5(code, companyName, contextSummary) {
  const company = companyName ? `「${companyName}」 (証券コード ${code})` : `証券コード ${code}`;
