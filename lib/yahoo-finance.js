...
async function fetchYahooChart(url, symbol) {
  ...
  // Volume — latest + 5D average
  const validVolumes = volumes.filter(v => v != null && v > 0);
  const lastVolume = meta.regularMarketVolume ?? (validVolumes.length > 0 ? validVolumes[validVolumes.length - 1] : null);
  const avgVolume5d = validVolumes.length >= 5
    ? Math.round(validVolumes.slice(-5).reduce((a, b) => a + b, 0) / 5)
    : null;
